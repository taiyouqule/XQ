...
public class ReasonerEstimateUserSubTree extends ReasonerEstimate {

	public ReasonerEstimateUserSubTree(ReasonerMachine state) {
		super(state);
	...
	public boolean esitimate(ReasonerInteractBean interactBean,ReasonerReturnBean returnBean){	
		boolean reBool=true;
		//用户推理树是FAQ推理是的子树
		if (interactBean.getRelevance()<0&&interactBean.getRelevance()>-1){
			...
			if(reasult.equals(InteractCommon.ResultClassify.Exact)){
				returnBean.getResultMap().put(InteractCommon.ReasultState.Exact_UserSub,interactBean.getFaqMessage());
			}
			else if(reasult.equals(InteractCommon.ResultClassify.NunAbstract)){
				returnBean.getResultMap().put(InteractCommon.ReasultState.NAbstract_L2,interactBean.getFaqMessage());
			}
			else if(reasult.equals(InteractCommon.ResultClassify.Abstract)){
				returnBean.getResultMap().put(InteractCommon.ReasultState.Abstract_L2,interactBean.getFaqMessage());
			}
			reBool=false;
		}
		return reBool;			
