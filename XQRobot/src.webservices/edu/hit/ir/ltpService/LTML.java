...
public class LTML {
	...
	//自定义添加，当SERVLET调用时使用
	public void buildWeb(String sourceStr) throws JDOMException, IOException {
		//String gbStr=new String(sourceStr.getBytes("UTF-8"),"UTF-8");
		// System.out.println("here");
		Reader xmlStreamReader=new InputStreamReader(new ByteArrayInputStream(sourceStr.getBytes()),"gbk");
	    //byte[] b = sourceStr.getBytes();
	   // InputStream ip = new ByteArrayInputStream(b);
		doc = xml.build(xmlStreamReader);
		root = doc.getRootElement().getChild(LTPTag.DOC);
		note = doc.getRootElement().getChild(LTPTag.NOTE);
		initIndex();
	}
